...
export const AdminSocialProjects: React.FC = () => {
  ...
  const handleOpenModal = (project?: SocialProject) => {
    if (project) {
        ...
        // Ordenação por timestamp descendente na galeria
        const sortedGallery = [...(rest.gallery || [])].sort((a, b) => b.registeredAt - a.registeredAt);
        setFormData({ ...rest, gallery: sortedGallery });
    } else {
        setEditingId(null);
  ...
  const handleDelete = async (id: string) => {
      ...
      if(confirm(`Excluir permanentemente "${project?.title}"?`)) {
          if (project?.linkedEventId) {
              await deleteDocument('events', project.linkedEventId);
          }
  ...
  const syncEvent = async (projectData: Omit<SocialProject, 'id'>, projectId: string) => {
      const eventData: Omit<ChurchEvent, 'id'> = {
          title: `IBOC Social: ${projectData.title}`,
          type: 'Social',
          start: `${projectData.date}T08:00:00`,
          end: `${projectData.date}T12:00:00`,
          location: projectData.location || 'Sede IBOC',
          description: projectData.description,
          bannerUrl: projectData.bannerUrl,
          linkedProjectId: projectId
      };

      if (projectData.linkedEventId) {
          try {
              await updateDocument('events', projectData.linkedEventId, eventData);
              return projectData.linkedEventId;
          } catch {
              // Se o evento vinculado não existir mais, cria um novo
              const newEvent = await addDocument('events', eventData);
              return newEvent.id;
          }
      } else {
          const newEvent = await addDocument('events', eventData);
      ...
      try {
          ...
          if (editingId) {
              if (formData.showInCalendar) {
                  const eventId = await syncEvent(projectToSave, editingId);
                  projectToSave.linkedEventId = eventId;
              } else if (formData.linkedEventId) {
                  await deleteDocument('events', formData.linkedEventId);
                  projectToSave.linkedEventId = '';
              }
              await updateDocument('social_projects', editingId, projectToSave);
          } else {
              const newDoc = await addDocument('social_projects', projectToSave);
              if (formData.showInCalendar) {
                  const eventId = await syncEvent(projectToSave, newDoc.id);
                  projectToSave.linkedEventId = eventId;
                  await updateDocument('social_projects', newDoc.id, projectToSave);
              }
          }
  ...
  const handleBatchImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
        ...
        const now = Date.now();
        const newItems: SocialProjectItem[] = newUrls.map((url, idx) => {
  ...
  const removeGalleryItem = (timestamp: number) => {
      setFormData(prev => ({
          ...prev,
          gallery: prev.gallery.filter(item => item.registeredAt !== timestamp)
      }));
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
            ...
            <p className="text-sm text-gray-500 font-sans tracking-tight">Gerenciamento editorial e integração com a agenda do site.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {loading ? (
              <div className="col-span-full py-32 flex flex-col items-center">
                  ...
                  <p className="text-gray-400 italic">Sincronizando acervo...</p>
              </div>
          ) : projects.length === 0 ? (
              <div className="col-span-full py-20 text-center text-gray-400 italic bg-white rounded-3xl border border-dashed border-gray-200">
                  Sem projetos. Use o botão "Novo Projeto" acima.
              </div>
          ) : projects.map(p => (
              <div key={p.id} className="bg-white rounded-3xl shadow-soft border border-gray-100 overflow-hidden flex flex-col group hover:shadow-xl transition-all duration-500">
                  <div className="h-48 relative overflow-hidden bg-navy-900">
                      ...
                      <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-md px-3 py-1 rounded-xl shadow-sm text-center border border-white/50">
                          <span className="block text-lg font-serif font-bold text-navy-900 leading-none">{new Date(p.date).getDate() + 1}</span>
                      ...
                      {p.showInCalendar && (
                          <div className="absolute bottom-4 right-4 bg-green-500 text-white p-1.5 rounded-full shadow-lg" title="Evento Ativo na Agenda">
                              <Globe size={14} />
                          </div>
                      )}
                  </div>
                  <div className="p-6 flex-1 flex flex-col">
                      <h3 className="font-serif font-bold text-xl text-navy-900 mb-2 group-hover:text-gold-600 transition-colors leading-tight line-clamp-1">{p.title}</h3>
                      ...
                      <div className="flex items-center gap-4 text-xs text-gray-400 mb-8 mt-auto">
                          <div className="flex items-center gap-2"><MapPin size={14} className="text-gold-500"/> {p.location || 'Sede'}</div>
                          <div className="flex items-center gap-2"><ImageIcon size={14} className="text-gold-500"/> {p.gallery?.length || 0} Fotos</div>
                      </div>
                      <div className="flex gap-3 pt-4 border-t border-gray-50">
                          <button onClick={() => handleOpenModal(p)} className="flex-1 py-2.5 bg-stone-50 hover:bg-navy-900 hover:text-white rounded-xl transition-all text-gray-600 font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-2">
              ...
              <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-4xl max-h-[92vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-500">
                  <div className="p-10 border-b border-gray-100 bg-rose-50/20 flex justify-between items-center">
                      <div className="flex items-center gap-6">
                        ...
                        <div>
                          <h2 className="text-3xl font-serif font-bold text-navy-900">{editingId ? 'Editar Projeto' : 'Registrar Projeto'}</h2>
                          <p className="text-xs text-rose-600 uppercase tracking-[0.3em] mt-1 font-bold">Relatório de Impacto Social</p>
                        </div>
                      </div>
                  ...
                  <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-10 space-y-12">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                          <div className="md:col-span-2 flex items-center justify-between bg-stone-50 p-6 rounded-2xl border border-stone-100 shadow-inner">
                              <div className="flex items-center gap-4">
                                  <div className={`p-3 rounded-xl ${formData.showInCalendar ? 'bg-navy-900 text-gold-500 shadow-glow' : 'bg-gray-200 text-gray-400'}`}>
                                      <CalendarRange size={24} />
                                  </div>
                                  <div>
                                      <h4 className="font-bold text-navy-900">Incluir Evento na Agenda do Site?</h4>
                                      <p className="text-xs text-gray-500">Cria automaticamente um card na agenda pública.</p>
                                  </div>
                              </div>
                              <label className="relative inline-flex items-center cursor-pointer">
                                  <input type="checkbox" className="sr-only peer" checked={formData.showInCalendar} onChange={e => setFormData({...formData, showInCalendar: e.target.checked})} />
                          ...
                          <div className="md:col-span-2">
                            ...
                            <input required className={inputClass} value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} placeholder="Título da Ação" />
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            <input className={inputClass} value={formData.location || ''} onChange={e => setFormData({...formData, location: e.target.value})} placeholder="Comunidade / Endereço" />
                          </div>
                          <div className="md:col-span-2">
                            <label className={labelClass}>Descrição / Relato</label>
                            <textarea className={inputClass + " h-32 resize-none shadow-inner bg-stone-50/50"} value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} placeholder="Objetivos e resultados alcançados..." />
                          </div>
                      </div>

                      {/* Banner Section */}
                      <div className="space-y-4">
                          <label className={labelClass}>Imagem de Capa (Destaque)</label>
                          <div className="relative h-60 rounded-3xl overflow-hidden bg-stone-100 border-2 border-dashed border-stone-200 group flex items-center justify-center shadow-inner">
                              {formData.bannerUrl ? (
                                  <>
                                  ...
                                  <label className="cursor-pointer flex flex-col items-center gap-3">
                                      {uploadingBanner ? <Loader2 className="animate-spin text-gold-500" size={32}/> : <Plus className="text-stone-300" size={48}/>}
                                      <span className="text-xs text-gray-400 font-bold uppercase tracking-widest">Adicionar Foto de Capa</span>
                                      <input type="file" className="hidden" accept="image/*" onChange={handleBannerUpload} />
                                  </label>
                              )}
                          </div>
                      ...
                      <div className="space-y-6">
                          <div className="flex justify-between items-center border-b border-stone-100 pb-4">
                              <h4 className="font-serif font-bold text-xl text-navy-900 italic">Galeria de Fotos</h4>
                              <p className="text-[10px] text-gray-400 uppercase tracking-widest bg-stone-100 px-3 py-1 rounded-full">Arraste múltiplos arquivos para upload</p>
                          </div>
                          
                          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                              <label className={`aspect-square rounded-2xl border-2 border-dashed border-gold-200 bg-gold-50/20 flex flex-col items-center justify-center cursor-pointer hover:bg-gold-50 transition-all group ${batchUploading ? 'opacity-50 pointer-events-none' : ''}`}>
                                  {batchUploading ? <Loader2 className="animate-spin text-gold-500" size={32}/> : <Upload className="text-gold-500 group-hover:scale-110 transition-transform" size={32}/>}
                                  <span className="text-[10px] font-bold text-gold-600 uppercase tracking-tighter mt-2 text-center px-2">{batchUploading ? 'Enviando...' : 'Adicionar Fotos'}</span>
                                  <input type="file" multiple accept="image/*" className="hidden" onChange={handleBatchImageUpload} />
                              </label>

                              {formData.gallery?.map((item) => (
                                  <div key={item.registeredAt} className="relative group aspect-square rounded-2xl overflow-hidden border border-stone-100 bg-white shadow-sm ring-1 ring-black/5">
                                      ...
                                      <div className="absolute inset-0 bg-navy-900/60 opacity-0 group-hover:opacity-100 transition-opacity p-3 flex flex-col justify-end">
                                          <p className="text-[8px] text-white italic leading-tight mb-2 line-clamp-2">"{item.verse}"</p>
                                          <button type="button" onClick={() => removeGalleryItem(item.registeredAt)} className="self-end p-2 bg-red-600 text-white rounded-lg shadow-xl"><X size={12}/></button>
                                      </div>
                                  </div>
                              ))}
                          </div>
                      ...
                      <div className="p-10 border-t border-stone-50 bg-stone-50/50 flex justify-end gap-6 rounded-b-[2.5rem]">
                          ...
                          <Button type="submit" disabled={submitting || uploadingBanner || batchUploading} className="px-20 py-4 rounded-2xl shadow-glow font-bold text-sm">
                              ...
                              {editingId ? 'Confirmar Edição' : 'Salvar Projeto'}
                          </Button>
                      </div>
                  </form>
              </div>
